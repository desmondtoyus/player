...
Player.prototype.adsManagerLoadedCallback = function () { var events = [google.ima.AdEvent.Type.ALL_ADS_COMPLETED, google.ima.AdEvent.Type.CLICK, google.ima.AdEvent.Type.COMPLETE, google.ima.AdEvent.Type.FIRST_QUARTILE, google.ima.AdEvent.Type.LOADED, google.ima.AdEvent.Type.MIDPOINT, google.ima.AdEvent.Type.PAUSED, google.ima.AdEvent.Type.STARTED, google.ima.AdEvent.Type.THIRD_QUARTILE]; for (var index = 0; index < events.length; index++) { this.playerz.ima.addEventListener(events[index], this.onAdEvent.bind(this)) } }; Player.prototype.onAdEvent = function (event) {
    console.log("EVENT", event.type); if (event.type == 'start') { pause(this.id); console.log('PAUSED'); let x = document.getElementById(this.id).offsetLeft, y = document.getElementById(this.id).offsetTop, w = document.getElementById(this.id).offsetWidth, h = document.getElementById(this.id).offsetHeight, r = x + w, b = y + h, visibleX, visibleY, visible; visibleX = Math.max(0, Math.min(w, window.pageXOffset + window.innerWidth - x, r - window.pageXOffset)); visibleY = Math.max(0, Math.min(h, window.pageYOffset + window.innerHeight - y, b - window.pageYOffset)); visible = visibleX * visibleY / (w * h); if (visible > fraction) { play(this.id); console.log('PLAY') } }
    if (event.type == 'allAdsCompleted') { let endingPlay = document.getElementById(this.id); endingPlay.style.visibility = 'hidden'; endingPlay.getElementsByClassName('vjs-control-bar')[0].style.visibility = 'hidden' }
}; var obj = document.getElementsByClassName("pilot-player"); var realHeight = []; var realWidth = []; for (let index = 0; index < obj.length; index++) { realHeight[index] = document.getElementsByClassName('pilot-player')[index].style.height; realWidth[index] = document.getElementsByClassName('pilot-player')[index].style.width; let id = obj[index].getAttribute('id'); let vastTag = obj[index].getAttribute("value"); console.log(id); console.log(vastTag); console.log('============================='); var player1 = new Player(id, vastTag); player1.init() }
var videos = document.getElementsByClassName("pilot-player"); var fraction = 0.6; let count = []
function checkScroll() {
    console.log('ID =', videos[0].getAttribute('id')); if (videos[0].getAttribute('data-view') !== "mobile_web") {
        for (let i = 0; i < videos.length; i++) {
            var playVideo = videos[i]; let playId = videos[i].getAttribute('id'); var x = playVideo.offsetLeft, y = playVideo.offsetTop, w = playVideo.offsetWidth, h = playVideo.offsetHeight, r = x + w, b = y + h, visibleX, visibleY, visible; visibleX = Math.max(0, Math.min(w, window.pageXOffset + window.innerWidth - x, r - window.pageXOffset)); visibleY = Math.max(0, Math.min(h, window.pageYOffset + window.innerHeight - y, b - window.pageYOffset)); visible = visibleX * visibleY / (w * h); if (visible > fraction) {
                if (!(document.getElementsByClassName('pilot-player')[i].classList.contains('pilot-checker')) && !(document.getElementsByClassName('pilot-video')[i].classList.contains('stuck'))) { play(playId); console.log('PLAY') }
                ...
                playVideo.style.height = realHeight[i] + "px"; playVideo.style.width = realWidth[i] + "px"; document.getElementsByClassName('pilot-video')[i].classList.remove("stuck"); document.getElementsByClassName('pilot-player')[i].classList.add("pilot-checker")
            } else { if ((document.getElementById(playId).classList.contains('pilot-checker'))) { let stuckHeight = Math.max(window.innerHeight) / 4 + "px"; let stuckWidth = Math.max(window.innerWidth) / 8 + "px"; playVideo.style.height = stuckHeight; document.getElementsByClassName('pilot-video')[i].classList.add("stuck"); document.getElementsByClassName('pilot-player')[i].classList.remove("pilot-checker") } }
        }
    }
}

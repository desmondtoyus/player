...
let addclose=document.getElementsByClassName('pilot-video');for(let index=0;index<addclose.length;index++){let closeBtn=document.createElement('div');closeBtn.innerHTML='&#10006;'
closeBtn.id=index;closeBtn.classList.add('pilot-closeBtn');closeBtn.addEventListener("click",function(e){let toClose=e.target.getAttribute("id")
document.getElementsByClassName('pilot-video')[toClose].style.display='none'});addclose[index].insertBefore(closeBtn,addclose[index].firstChild)}
...
{let slidingTotal=document.getElementsByClassName('pilot-video slider');for(let index=0;index<slidingTotal.length;index++){document.getElementsByClassName('pilot-video slider')[index].style.display='block';document.getElementsByClassName('pilot-video slider')[index].style.right='-350px'
console.log('Trying to move');pilotSlider()}}
function pause(id){var player=videojs(id);player.ima.pauseAd()}
function play(id){var player=videojs(id);player.ima.resumeAd()}
var Player=function(id,vastTag,inArticle){this.id=id;this.inArticle=inArticle;this.console=document.getElementById('ima-sample-console');this.playerz=videojs(id);this.init=function(){var player=videojs(this.id);var options={id:id,adTagUrl:vastTag,adsManagerLoadedCallback:this.adsManagerLoadedCallback.bind(this),preload:'auto'};player.ima(options);var contentPlayer=document.getElementById(id+'_html5_api');if((navigator.userAgent.match(/iPad/i)||navigator.userAgent.match(/Android/i))&&contentPlayer.hasAttribute('controls')){contentPlayer.removeAttribute('controls')}
